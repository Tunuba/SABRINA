...
DW="$(ruta_wsl ../decomp)"   # la ruta vista desde WSL
echo "== lote auto.py con $P procesos"
wsl -d "$SABRINA_WSL" -- bash -lc "cd \"$DW\" && . ~/decomp-herramientas/venv/bin/activate && python3 auto.py --procesos $P > build/auto_log.txt 2>&1; tail -n 5 build/auto_log.txt"
echo "== FIN"
